...
describe("#154: the pointer is marked on the canvas", () => {
  ...
  // Far enough off the spawn that a mark at the avatar rather than at the pointer fails, and inside
  // the 800 × 600 viewport `recordFrames` stands up.
  const POINTER = { x: 430, y: 320 };
  const POINTER_TILE = tileOf(POINTER);

  // Where the aim mark was struck, read back out of the ink. It is the last path the frame strokes,
  // and it is symmetric about the point it marks, so the middle of its extent is that point.
  function drawnAim(calls: DrawnCall[], into: HTMLElement): Vec2 | null {
  ...
  test("the mark stands on the point the pointer is on", async () => {
    const canvas = renderMatch();
    fireEvent.mouseMove(canvas, { clientX: POINTER.x, clientY: POINTER.y });
    await nextFrames();
    const at = drawnAim(drawn.calls, canvas) as Vec2;
    expect(at.x).toBeCloseTo(POINTER.x, 6);
    expect(at.y).toBeCloseTo(POINTER.y, 6);
  });

  // The whole of the ask: the point a shot is aimed at is legible without the OS cursor. `aimDir` is
  // what a shot is fired along, so the mark is the aim only if the two agree — and the mark is read
  // off the canvas rather than off the number it was drawn from.
  test("a shot is aimed at the point the mark stands on", async () => {
    const world = armed();
    const shots: Vec2[] = [];
    ...
    const toMark = unit(self, drawnAim(drawn.calls, canvas) as Vec2);
    expect(shots).toHaveLength(1);
    expect(shots[0].x).toBeCloseTo(toMark.x, 6);
    expect(shots[0].y).toBeCloseTo(toMark.y, 6);
  ...
  // read. A mark on one point and a click on another would be a worse cursor than the arrow.
  test("the tile a click mines is the tile the mark stands on", async () => {
    ...
    expect(tileOf(drawnAim(drawn.calls, canvas) as Vec2)).toEqual(POINTER_TILE);
  });
