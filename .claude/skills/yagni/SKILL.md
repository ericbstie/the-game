...
name: yagni
description: >-
  Climb a short decision tree before implementing anything, rooted in YAGNI
  (you aren't gonna need it). Auto-invoke before writing or generating code,
  adding a dependency, or introducing an abstraction: stop at the first rung
  that fully solves the problem. Never shortcut understanding, validation,
  error handling, security, or accessibility.
---

# YAGNI — decide before you implement

You aren't gonna need it. The best code is the code you never wrote. Before
writing anything, climb the tree and stop at the first rung that solves the
problem.
...
The tree shortens the *solution*, never the *reading*. Trace the whole thing
first — every file the change touches and the real flow — before picking a rung.
Skipping the understanding is how you end up writing more.
...
- No speculative abstractions, no scaffolding "for later," no config nobody asked for. Solve today's problem, not an imagined future one.
